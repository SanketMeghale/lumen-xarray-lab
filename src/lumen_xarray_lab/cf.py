...
def get_coordinate_metadata(dataset: xr.Dataset) -> dict[str, dict[str, Any]]:
    ...
    for name, coord in dataset.coords.items():
        ...
        entry: dict[str, Any] = {
            ...
            "queryable": bool(coord.ndim == 1),
            "curvilinear": bool(coord.ndim > 1 and role in {"latitude", "longitude"}),
            "map_candidate": bool(role in {"latitude", "longitude"} and coord.ndim in {1, 2}),
